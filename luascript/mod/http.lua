module("mod.http", package.seeall)
...
local USERAGENT = 'mod.http/' .. _VERSION
...
-- global variables
local url = require("mod.url")

local mt = { __index = mod.http }
...
function urlencode(text)
	return text:gsub("[^0-9A-Za-z]", _fix);
end
...
getmetatable(mod.http).__newindex = function (table, key, val)
