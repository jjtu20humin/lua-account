...
function check_param ( cmd, _GET, _POST, _FILE )
	...
	for __k,v in pairs( _pattern.base_param ) do
		local k = v.name;
		local _value;
		if _pattern.UPLOAD_TYPE == "GET" then
			_value = get_table_value(_GET[k] );
		elseif _pattern.UPLOAD_TYPE == "BOTH" then
			_value = get_table_value(_GET[k] ) or get_table_value( _POST[k] );
		else
			_value = get_table_value( _POST[k] );
		end
